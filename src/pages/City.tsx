...
const reviewsFor = (cityPrepositional: string) => [
  {
    name: 'Игорь Соколов',
    role: 'ИП, сфера услуг',
  ...
  },
  {
    ...
    text: `Заказывала гостиничные чеки в ${cityPrepositional} для отчёта — всё оказалось быстро и без лишних вопросов. Приятно, что можно было согласовать детали в Telegram, не отрываясь от дел.`,
    rating: 5,
  },
  {
    name: 'Дмитрий Волков',
    role: 'Менеджер по снабжению',
    text: `Пользуемся услугами компании уже больше года для оформления товарных чеков в ${cityPrepositional}. Стабильно хорошее качество бланков, доставка точно в срок по всему городу.`,
    rating: 5,
  },
  {
    name: 'Елена Прохорова',
    role: 'Бухгалтер',
    text: `Очень внимательное отношение к деталям — все реквизиты и суммы сходятся с первого раза. Рекомендую коллегам в ${cityPrepositional}, кто ищет надёжного исполнителя по отчётным документам.`,
    rating: 5,
  },
];
...
const City = () => {
  ...
  const REVIEWS = reviewsFor(city.prepositional);
